...
export const payslipComponentsRouter = router({
  list: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      const rows = await ctx.db
        ...
        .where(isNull(payslipComponentType.archivedAt))
  ...
  create: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [result] = await ctx.db
        ...
        .from(payslipComponentType)
        .where(eq(payslipComponentType.ownerId, input.ownerId))
      ...
      const [inserted] = await ctx.db
        .select()
        .from(payslipComponentType)
        .where(eq(payslipComponentType.id, id))
      if (!inserted) {
  ...
  update: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await ctx.db.update(payslipComponentType).set(setFields).where(eq(payslipComponentType.id, id))
  ...
  archive: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const [target] = await ctx.db
        .select()
        .from(payslipComponentType)
        .where(eq(payslipComponentType.id, input.id))
      if (!target) {
        throw new TRPCError({ code: 'NOT_FOUND', message: 'Component not found' })
      }
      const now = Date.now()
